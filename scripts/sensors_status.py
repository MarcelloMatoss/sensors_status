...
class Topic_create:
    def __init__(self, topics_name, msgs_types, topics_frequency, fequency_tolerance, max_downtime, topic_status_pub):
        ...
        for i in range(self.topics_num):
            self.topics.append(Sensor_status(self.topics_name[i], self.msgs_types[i], self.topics_frequency[i],self.fequency_tolerance[i], self.max_downtime, self.topic_status_pub,i))

class Sensor_status:

    def __init__(self, topic_name, msg_type, topic_frequency, fequency_tolerance, max_downtime, topic_pub, ind):
        ...
        self.ind = ind
        self.pub = rospy.Publisher(self.topic_pub, ByteMultiArray, queue_size=10, latch=True)
        self.msg = ByteMultiArray()
        self.status = self.msg.data
        self.counter = 0
        self.status_topic = False
        self.msg_class = get_message_class(self.msg_type) 
        self.time_init = rospy.get_rostime().secs + rospy.get_rostime().nsecs*(10**(-9))

        rospy.Subscriber(self.topic_pub, ByteMultiArray, self.msg_callback, queue_size=10)
    ...
    def topic_hz_check(self, data):
        self.counter += 1
        if self.counter == self.topic_frequency:
            time_final = rospy.get_rostime().secs + rospy.get_rostime().nsecs*(10**(-9))
            rate = self.topic_frequency/ (time_final - self.time_init)
    ...
    def status_check(self, event):
        time_final = rospy.get_rostime().secs + rospy.get_rostime().nsecs*(10**(-9))
        if (time_final - self.time_init) > self.max_downtime:
            if self.status_topic:
                self.status_topic = False
                print(self.status_topic)
                self.status[self.ind] = False
...
if __name__ == "__main__":
    try:
        ...
        for sensor in sensors:
            Topic_create(sensor['topics_sensor'], sensor['msgs_types'], sensor['topics_frequecy'], sensor['fequency_tolerance'], sensor['max_downtime'], sensor['status_topic_pub'])
        rospy.spin()
