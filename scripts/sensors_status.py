...
class Sensor_status:
    def __init__(self, topic_name, msg_type, topic_frequency, sampling, fequency_tolerance, max_downtime, topic_status_pub, ind):
        rospy.loginfo('Starting verification [topic: %s]', topic_name) 
        ...
        self.pub = rospy.Publisher(self.topic_status_pub, Float32MultiArray, queue_size=10, latch=True)
        self.msg = Float32MultiArray()
        self.status = self.msg.data
        self.status_topic = 0.0

        self.msg_class = get_message_class(self.msg_type)
        self.time_init = rospy.get_time()
        self.downtime_init = rospy.get_time()
        
        rospy.Subscriber(self.topic_status_pub, Float32MultiArray, self.msg_callback, queue_size=10)
    ...
    def topic_hz_check(self, data):
        ...
        if self.counter == self.sampling:
            ...
            if rate <= (self.topic_frequency + (self.topic_frequency*self.fequency_tolerance)) and rate >= (self.topic_frequency - (self.topic_frequency*self.fequency_tolerance)):
                if not (self.status_topic == 1.0):
                    self.status_topic = 1.0
                    self.status[self.ind] = 1.0
                    self.msg.data = self.status
                    self.pub.publish(self.msg) 
                    rospy.loginfo("\033[92m"  + 'ok frequency [topic: %s]' + "\033[0m", self.topic)        
            else:
                if rate > (self.topic_frequency + (self.topic_frequency*self.fequency_tolerance)):
                    
                    if not (self.status_topic == 1.5):
                        rospy.logwarn('high frequency [topic: %s]', self.topic)
                        self.status_topic = 1.5
                        self.status[self.ind] = 1.5
                        self.msg.data = self.status
                        self.pub.publish(self.msg)
                else:
                    if not (self.status_topic == 0.5): 
                        rospy.logwarn('low frequency [topic: %s]', self.topic)
                        self.status_topic = 0.5
                        self.status[self.ind] = 0.5
                        self.msg.data = self.status
                        self.pub.publish(self.msg)   
    ...
    def status_check(self, event):
        if (rospy.get_time() - self.downtime_init) > self.max_downtime:
            if not (self.status_topic == 0.0):
                self.status_topic = 0.0
                self.status[self.ind] = 0.0
                self.msg.data = self.status
                self.pub.publish(self.msg)
                rospy.logerr('[topic: %s]', self.topic)
...
if __name__ == "__main__":
    try:
        ...
        for sensor in sensors:
            ...
            Topic_create(sensor['topics_sensor'], 
                         sensor['msgs_types'], 
                         sensor['topics_frequency'], 
                         sensor['sampling'], 
                         sensor['frequency_tolerance'], 
                         sensor['max_downtime'], 
                         sensor['status_topic_pub'])
        rospy.spin()
